...
from tensorflow.keras.models import load_model, save_model # For full model save/load if preferred later
warnings.filterwarnings('ignore')

class FitRecSpeedPredictor:
    def __init__(self, window_duration=3, prediction_duration=3):
        """
    ...
    def load_endomondo_data(self):
        ...
        try:
            path = Path("data/")
            out_path = str(path / "raw_run_data.npy") # Assuming this path exists and contains the data
    ...
    def process_endomondo_dict(self, data, session_id=None):
        ...
        try:
            ...
            required_fields = ['heart_rate', 'speed', 'timestamp']
    ...
    def create_model(self, input_shape):
        ...
        model = Sequential([
            ...
            LSTM(32, return_sequences=False,
                 ...
                 recurrent_regularizer=regularizers.l2(0.001)),
            Dropout(0.3),
            # BatchNormalization(), # Removed BN after last LSTM before Dense for stability with some TF versions
                                 # Or ensure Dense layers are robust enough.
    ...
    def save_model_config(self, config_path="model.json", weights_filename="model_weights.weights.h5"):
        """
        Saves the model configuration, scaler states, and model weights.
        Args:
            config_path (str): Path to save the JSON configuration file.
            weights_filename (str): Filename for the model weights (will be saved relative to config_path).
        """
        if self.model is None:
            print("Model has not been trained or created yet. Cannot save.")
            return

        config_dir = Path(config_path).parent
        config_dir.mkdir(parents=True, exist_ok=True) # Ensure directory exists
        full_weights_path = config_dir / weights_filename

        config = {
            "model_type": "FitRecSpeedPredictor",
            "window_duration": self.window_duration,
            "prediction_duration": self.prediction_duration,
            "sampling_rate": self.sampling_rate,
            "model_weights_path": weights_filename, # Store relative path or just filename
            "scalers": {
                "speed_scaler": {
                    "min_": self.speed_scaler.min_.tolist() if hasattr(self.speed_scaler, 'min_') else None,
                    "scale_": self.speed_scaler.scale_.tolist() if hasattr(self.speed_scaler, 'scale_') else None,
                },
                "hr_scaler": {
                    "min_": self.hr_scaler.min_.tolist() if hasattr(self.hr_scaler, 'min_') else None,
                    "scale_": self.hr_scaler.scale_.tolist() if hasattr(self.hr_scaler, 'scale_') else None,
                },
                "target_scaler": {
        ...
        try:
            ...
            self.model.save_weights(str(full_weights_path))
            print(f"Model weights saved to {full_weights_path}")

        except Exception as e:
            print(f"Error saving model config or weights: {e}")

    def load_model_config(self, config_path="model.json"):
        """
        Loads the model configuration, scaler states, and model weights.
        Args:
        ...
        try:
            ...
            self.prediction_duration = config["prediction_duration"]
            self.sampling_rate = config["sampling_rate"]

            if self.sampling_rate <= 0:
                print(f"Warning: Loaded sampling rate is {self.sampling_rate}. Setting to 1 as a fallback.")
                self.sampling_rate = 1.0


            # Load scalers
            speed_scaler_config = config["scalers"]["speed_scaler"]
            self.speed_scaler.min_ = np.array(speed_scaler_config["min_"]) if speed_scaler_config["min_"] is not None else None
            self.speed_scaler.scale_ = np.array(speed_scaler_config["scale_"]) if speed_scaler_config["scale_"] is not None else None
            # For MinMaxScaler, need to set n_features_in_ and feature_names_in_ if not set by partial_fit or fit
            if self.speed_scaler.min_ is not None:
                 self.speed_scaler.n_features_in_ = len(self.speed_scaler.min_)
            # Similar for other scalers if they were fitted
            # self.speed_scaler.feature_names_in_ = np.array([f'feature_{i}' for i in range(self.speed_scaler.n_features_in_)], dtype=object)


            hr_scaler_config = config["scalers"]["hr_scaler"]
            self.hr_scaler.min_ = np.array(hr_scaler_config["min_"]) if hr_scaler_config["min_"] is not None else None
            self.hr_scaler.scale_ = np.array(hr_scaler_config["scale_"]) if hr_scaler_config["scale_"] is not None else None
            if self.hr_scaler.min_ is not None:
                self.hr_scaler.n_features_in_ = len(self.hr_scaler.min_)


            target_scaler_config = config["scalers"]["target_scaler"]
            self.target_scaler.min_ = np.array(target_scaler_config["min_"]) if target_scaler_config["min_"] is not None else None
            self.target_scaler.scale_ = np.array(target_scaler_config["scale_"]) if target_scaler_config["scale_"] is not None else None
            if self.target_scaler.min_ is not None:
                self.target_scaler.n_features_in_ = len(self.target_scaler.min_)


            # Recreate model and load weights
            window_samples = int(self.window_duration / self.sampling_rate)
            if window_samples <= 0:
                raise ValueError(f"Calculated window_samples ({window_samples}) is invalid. Check window_duration and sampling_rate.")

            num_features = 2 # speed, heart_rate
            input_shape = (window_samples, num_features)
            self.model = self.create_model(input_shape=input_shape)

            weights_filename = config["model_weights_path"]
            full_weights_path = config_p.parent / weights_filename
            self.model.load_weights(str(full_weights_path))
            print(f"Model configuration and weights loaded from {config_path} and {full_weights_path}")

        except FileNotFoundError:
            print(f"Error: Configuration file not found at {config_path}")
            raise
        except Exception as e:
            print(f"Error loading model config or weights: {e}")
            raise
...
def main():
    ...
    predictor = FitRecSpeedPredictor(window_duration=6, prediction_duration=6) # Use window_duration and prediction_duration
    ...
    if results is None:
        ...
        if predictor is not None: # Save even if results are None but predictor exists
             predictor.save_model_config() # Save the trained model config and weights
    ...
    # Save the trained model config and weights
    if predictor.model is not None:
        predictor.save_model_config(config_path="model.json", weights_filename="model_weights.weights.h5")
    else:
